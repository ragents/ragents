...
function RAgent_onEvent(message) {
  ...
  if (message.from == "server") {
    if ((message.name == "agentCreated") || (message.name == "agentDestroyed")) {
      agent = this._session._ragents.get(body.id)
      if (null == agent) {
        var agent = new RAgent(this._session, body)
        this._session._ragents.set(body.id, agent)
      }

      this._session.emit(name, agent)
    }
    else {
      this._session.emit(name, body)
    }
  }
  else {
    this.emit(name, body)
