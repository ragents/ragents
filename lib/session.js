...
function Session(channel, opts, cb) {
  ...
  this._ragentServer.addListener("ragentCreated",   onRAgentCreated)
  this._ragentServer.addListener("ragentDestroyed", onRAgentDestroyed)
  ...
  function onMessage(message) {
    ...
    if (message.to == "server") {
      agent = session._ragentServer
    }
    else if (message.type == "request") {
      agent = session._agents.get(message.to)
    }
    else {
      agent = session._ragents.get(message.to)
    }
